...
def _create_retina(n_retina: int) -> nn.ModuleList:
    """Cria camada Retina com todos os mecanismos ativos."""
...
def _create_lgn(n_lgn: int) -> nn.ModuleList:
    """Cria camada LGN com todos os mecanismos ativos."""
...
def _create_v1(n_v1: int) -> nn.ModuleList:
    """Cria camada V1 com todos os mecanismos ativos."""
    cfg = _base_full_cfg(n_synapses_per_dendrite=8, theta_init=0.45, lateral_strength=0.3)
    return nn.ModuleList(MPJRDNeuronAdvanced(cfg) for _ in range(n_v1))
