...
| --- | --- | --- |
| `2.x` (janela de migração estendida) | Aliases v1 continuam disponíveis com `DeprecationWarning`. | Compatibilidade para upgrade gradual sem quebra imediata. |
| `3.0.0` (versão-limite planejada) | Aliases v1 são removidos da superfície pública. | Importações legadas passam a falhar, exigindo nomes v2. |

Critérios objetivos para a remoção:

1. **Ciclos mínimos:** manter aliases por, no mínimo, toda a janela de migração da série `2.x` antes da remoção.
2. **Aviso em changelog:** registrar explicitamente em `CHANGELOG.md` que aliases v1 serão removidos em `3.0.0`.
...
4. Enquanto `pyfolds.__version__ < 3.0.0`, acesso a aliases v1 deve seguir emitindo `DeprecationWarning` (coberto por teste de contrato).
