# PyFolds Development Hub

> Portal centralizado de navegação da documentação técnica e operacional do PyFolds.

## 🎯 Escolha seu perfil

### 👨‍💻 Desenvolvedor(a)
- [Instalação (5 min)](_quickstart/README.md)
- [Quickstart](_quickstart/quickstart.md)
- [Conceitos centrais](public/guides/core_concepts.md)
- [Padrões de engenharia](public/guides/engineering_patterns.md)
- [Referência de API](api/core.md)
- [Tutoriais e exemplos](tutorials/README.md)

### 🏛️ Arquiteto(a)
- [Especificações técnicas](specifications/README.md)
- [Arquitetura de alto nível](ARCHITECTURE.md)
- [Blueprints e diagramas](architecture/blueprints/README.md)
- [Histórico de decisões (ADRs)](governance/adr/INDEX.md)

### 🔬 Pesquisador(a)
- [Portal científico](science/README.md)
- [Lógica científica](science/SCIENTIFIC_LOGIC.md)
- [Metodologia](science/METHODOLOGY.md)
- [Protocolo de testes](science/TEST_PROTOCOL.md)
- [Pesquisa MPJRD](research/mpjrd/mpjrd_model.md)

### 👥 PM / Maintainer
- [Hub de controle de execução](development/HUB_CONTROLE.md)
- [Plano mestre](governance/MASTER_PLAN.md)
- [Qualidade e auditoria](governance/quality/ieee_code_audit_mpjrd.md)
- [Registro de riscos](governance/RISK_REGISTER.md)
- [Processo de release](development/release_process.md)

## 📚 Mapa completo de seções

- [_quickstart/](./_quickstart/) — entrada rápida.
- [specifications/](./specifications/) — especificações técnicas.
- [guides/](./guides/) — índice lógico para guias públicos.
- [tutorials/](./tutorials/) — índice lógico para exemplos.
- [api/](./api/) — documentação de API.
- [science/](./science/) — teoria, metodologia e validação.
- [architecture/](./architecture/) — blueprints e evolução.
- [governance/](./governance/) — ADRs, qualidade, riscos.
- [development/](./development/) — contribuição e operação interna.

## 🔗 Atalhos de tarefas

| Tarefa | Documento |
|---|---|
| Instalar e rodar primeiro exemplo | [`_quickstart/README.md`](_quickstart/README.md) |
| Entender o modelo MPJRD | [`science/README.md`](science/README.md) |
| Consumir API | [`api/core.md`](api/core.md) |
| Contribuir | [`development/README.md`](development/README.md) |
| Acompanhar governança | [`governance/README.md`](governance/README.md) |
| Acompanhar issues/documentação interna | [`development/HUB_CONTROLE.md`](development/HUB_CONTROLE.md) |

---

Voltar ao portal principal: [docs/README.md](README.md)
