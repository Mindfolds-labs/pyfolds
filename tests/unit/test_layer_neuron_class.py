...
def test_layer_accepts_neuron_v2():
    ...
    layer = MPJRDLayer(n_neurons=3, cfg=cfg, neuron_class=MPJRDNeuronV2)

    x = torch.randn(5, 3, 2, 4)
    out = layer(x)

    assert out["spikes"].shape == (5, 3)
    assert layer.neuron_class is MPJRDNeuronV2
...
def test_layer_rejects_invalid_neuron_class():
    ...
    with pytest.raises(TypeError):
        MPJRDLayer(n_neurons=2, cfg=cfg, neuron_class=torch.nn.Linear)
